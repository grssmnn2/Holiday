...
class Friendlist extends React.Component {
  ...
  componentDidMount() {
    API.retrieveFriendList(this.props.email)
      .then(res => {
        console.log(res);
        this.setState(
          {
          ...
          () => {
            let unreadMessage = {};
            API.retrieveUnreadMessage(this.props.email)
              .then(result => {
  ...
  showChatbox = user => {
    if (this.state.className === null) {
      API.updateMessage(user, this.props.email)
        .then(data => {
        ...
        res.data.forEach(element => {
          obj = [
            ...obj,
            {
              ...
              time: element.created.substring(0, 19)
            }
          ];
        });
        console.log("hello");
  ...
  render() {
    ...
    return (
      <div>
        <Navbar
          pendingTrip={this.props.pendingTrip}
          number={this.state.messageNumber}
          authenticated={this.props.authenticated}
          click={this.display}
        />
        <Chatbox
          addNumber={name => this.addNumber(name)}
          authenticated={this.props.authenticated}
          messages={this.state.messages}
          isOpen={this.state.historyMessage}
              ...
              <List
                ...
                renderItem={item => (
                  <List.Item>
                    <List.Item.Meta
                      avatar={
                        <Avatar src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSWGh_yZxpnGqUgbbGQsbXVK0RQWVWr_Qgz3hlAKzZJZiOvFN8m" />
                      }
                      title={<a href="https://ant.design">{item.name}</a>}
                      description={item.email}
                    />
                    <div onClick={() => this.showChatbox(item.email)}>
                      {unreadUser[item.email] === true ? (
                        <GoMailRead style={{ fontSize: 30, color: "gold" }} />
                      ) : (
                        <IoIosChatbubbleOutline style={{ fontSize: 30 }} />
                      )}
                    </div>
                  </List.Item>
                )}
